  ...
  if (checkPresent.length == 0) {
    if (image) {
      ...
      if (imageFormat.find((value) => value == image.type)) {
        ...
        await db.run("INSERT INTO actions_article (id_article, id_utente, action, titolo, testo, image) VALUES(?, ?, ?, ?, ?, ?)", idPost, requestAuthorId, 1, title, content, `https://postimage.playxdefiant.it/${randomFileName + "_" + idPost}.webp`);
      } else {
